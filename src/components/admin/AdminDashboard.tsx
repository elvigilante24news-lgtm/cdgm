...
export function AdminDashboard() {
  ...
  const handleEditUser = (usuario: AuthUser) => { setSelectedUser(usuario); setIsEditDialogOpen(true); };
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        {usuario.estadoPago === 'deuda' && (
                          <>
                            <Button variant="ghost" size="sm" onClick={() => actualizarEstadoPago(String(usuario.id), 'al_dia')} title="Marcar pagado"><CheckCircle className="w-4 h-4 text-emerald-500" /></Button>
                            <Button variant="ghost" size="sm" onClick={() => handleEnviarNotificacion(String(usuario.id), 'pago')} title="Recordatorio"><Bell className="w-4 h-4 text-amber-500" /></Button>
                          </>
                        )}
                        {usuario.estado === 'activo' && (<Button variant="ghost" size="sm" onClick={() => actualizarEstadoUsuario(String(usuario.id), 'suspendido')} title="Suspender"><Ban className="w-4 h-4 text-red-500" /></Button>)}
                        {usuario.estado === 'suspendido' && (<Button variant="ghost" size="sm" onClick={() => actualizarEstadoUsuario(String(usuario.id), 'activo')} title="Activar"><Power className="w-4 h-4 text-emerald-500" /></Button>)}
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
