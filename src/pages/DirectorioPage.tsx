...
export default function DirectorioPage() {
  ...
  useEffect(() => { fetchDirectorio(); }, []);

  const usuariosFiltrados = useMemo(() => {
    return usuarios.filter((u) => {
      const matchName = `${u.nombre} ${u.apellido}`.toLowerCase().includes(searchName.toLowerCase());
      const matchMatricula = !searchMatricula || (u.numeroMatricula?.toLowerCase().includes(searchMatricula.toLowerCase()) ?? false);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#F0F5FF] to-white">
      ...
      <motion.header initial={{ y: -100 }} animate={{ y: 0 }} className="bg-white/80 backdrop-blur-xl shadow-lg shadow-sky-100/50 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <Link to="/"><Button variant="outline" className="border-[#0ea5e9] text-[#0ea5e9]">Portal matriculados</Button></Link>
        </div>
      </motion.header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
            Directorio de <span className="text-[#0ea5e9]">Profesionales</span>
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Encontrá diseñadores gráficos matriculados en la provincia de Misiones.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl p-6 shadow-lg shadow-gray-100 border border-gray-100 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar por nombre..." value={searchName} onChange={(e) => setSearchName(e.target.value)} className="pl-10" />
            </div>
            <div className="relative">
              <Shield className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <Input placeholder="N° de matrícula..." value={searchMatricula} onChange={(e) => setSearchMatricula(e.target.value)} className="pl-10" />
            </div>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            {(searchName || searchMatricula || (searchCiudad && searchCiudad !== 'todas')) && (
        ...
        {error && !isLoading && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <Button onClick={fetchDirectorio} variant="outline" className="border-[#0ea5e9] text-[#0ea5e9]">Reintentar</Button>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {usuariosFiltrados.map((usuario, index) => (
              <motion.div
                ...
                className="bg-white rounded-2xl p-6 shadow-lg shadow-gray-100 border border-gray-100 hover:shadow-xl transition-shadow"
              >
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {usuario.estudio && (
                      <p className="text-[#0284c7] font-medium text-sm flex items-center gap-1">
                        <Building2 className="w-3 h-3 flex-shrink-0" />
                        <span className="truncate">{usuario.estudio}</span>
                      </p>
                    )}
                    <p className="text-[#0ea5e9] font-medium text-sm">{usuario.numeroMatricula}</p>
                    <div className="flex items-center gap-1 text-gray-500 text-sm mt-1">
                      <MapPin className="w-3 h-3 flex-shrink-0" />{usuario.ciudad}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2 text-gray-600 text-sm">
                    ...
                    <a href={`mailto:${usuario.email}`} className="hover:text-[#0ea5e9] truncate">{usuario.email}</a>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600 text-sm">
                    <Phone className="w-4 h-4 text-gray-400 flex-shrink-0" />
                    <a href={`tel:${usuario.celular}`} className="hover:text-[#0ea5e9]">{usuario.celular}</a>
                  </div>
                </div>
